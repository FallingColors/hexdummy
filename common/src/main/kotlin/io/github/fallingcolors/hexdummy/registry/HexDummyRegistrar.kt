package io.github.fallingcolors.hexdummy.registry
...
typealias RegistrarEntry<T> = HexDummyRegistrar<T>.Entry<out T>

abstract class HexDummyRegistrar<T : Any>(
    ...
    fun <V : T> register(name: String, builder: () -> V): Entry<V> = register(HexDummy.id(name), builder)
    ...
    open inner class Entry<V : T>(
        ...
        override fun equals(other: Any?) = when (other) {
            is HexDummyRegistrar<*>.Entry<*> -> key.registry().equals(other.key.registry()) && id.equals(other.id)
            else -> false
        }
